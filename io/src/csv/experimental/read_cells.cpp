...
traccc::cell_module get_module(traccc::io::csv::cell c,
                               const traccc::digitization_map* dconfig) {
    ...
    if (dconfig != nullptr) {
        ...
        const auto volume_id = result.surface_link.volume();
        const auto& binning_data = dconfig->at(volume_id).binningData();
...
namespace traccc::io::csv::experimental {
...
void read_cells(cell_reader_output& out, std::string_view filename,
    ...
    while (reader.read(iocell)) {
        ...
        if (rit == result_modules.rend()) {
            ...
            const cell_module mod = get_module(iocell, dconfig);
